...
fn claim_transitions_to_claimed() {
    ...
    let (job, _lease) = space.claim("ocr", 30.0, None, now()).unwrap();
    assert_eq!(job.id, id);
...
fn claim_returns_none_when_queue_empty() {
    let space = TupleSpace::new();
    assert!(space.claim("ocr", 30.0, None, now()).is_none());
}
...
fn complete_marks_job_done() {
    let space = TupleSpace::new();
    let id = space.enqueue("ocr".into(), json!({}), 3, now());
    let (job, lease) = space.claim("ocr", 30.0, None, now()).unwrap();
...
fn complete_rejects_wrong_lease() {
    ...
    let space = TupleSpace::new();
    let id = space.enqueue("ocr".into(), json!({}), 3, now());
    let (job, _real_lease) = space.claim("ocr", 30.0, None, now()).unwrap();
...
fn expired_lease_returns_job_to_pending() {
    let space = TupleSpace::new();
    let id = space.enqueue("ocr".into(), json!({}), 3, now());
    let (_job, _lease) = space.claim("ocr", 10.0, None, now()).unwrap(); // expires at now+10
...
fn non_expired_lease_not_reaped() {
    let space = TupleSpace::new();
    space.enqueue("ocr".into(), json!({}), 3, now());
    space.claim("ocr", 30.0, None, now()).unwrap();
