...
fn apply(space: &TupleSpace, op: EventOp, timestamp: f64) {
    match op {
        ...
        EventOp::Claim { job_id, lease_id, lease_until, worker_id } => {
            space.apply_claim(JobId(job_id), LeaseId(lease_id), lease_until, timestamp, worker_id);
        }
        ...
        EventOp::Purge { job_id } => {
            space.remove(&JobId(job_id));
        }
        EventOp::Retry { job_id } => {
            let _ = space.retry(&JobId(job_id), timestamp);
        }
    }
}
