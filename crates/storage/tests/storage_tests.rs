...
fn claim_complete_lifecycle() {
    ...
    let (job, lease) = space.claim("ocr", 30.0, None).unwrap().unwrap();
...
fn fail_retries_then_fails_permanently() {
    ...
    let (j1, l1) = space.claim("ocr", 30.0, None).unwrap().unwrap();
    ...
    let (j2, l2) = space.claim("ocr", 30.0, None).unwrap().unwrap();
...
fn crash_recovery_with_snapshot() {
    ...
    let (id1, id2) = {
        ...
        let (j, l) = space.claim("ocr", 30.0, None).unwrap().unwrap();
...
fn expired_lease_is_replayed_correctly() {
    ...
    let job_id = {
        ...
        let (_job, _lease) = space.claim("ocr", -1.0, None).unwrap().unwrap();
        space.expire_leases();
        id
    };
